...
#ifdef USE_FREERTOS
#include "cmsis_os2.h"
extern osMutexId_t uds_mutexHandle;
extern osMessageQueueId_t slcan_info_to_pc_qHandle;
extern osMessageQueueId_t can_info_to_pc_qHandle;
#endif
...
#else
        slcan_byte_packets.can_data[i] = strtol(can_data_str, NULL, 16);
#endif
    }
#ifndef USE_FREERTOS
...
#else
    slcan_byte_packets.CAN_TxHeader.StdId = can_id;
    slcan_byte_packets.CAN_TxHeader.DLC = can_dlc;
    slcan_byte_packets.CAN_TxHeader.RTR = CAN_RTR_DATA;
    slcan_byte_packets.CAN_TxHeader.IDE = CAN_ID_STD;
    osMessageQueuePut( can_info_to_pc_qHandle, &slcan_byte_packets, 0u, 0u );
#endif
    return HAL_OK;
}
...
static uint8_t slcan_pc_send_extended_frame_to_bus( uint8_t* cmd_buff)
{
    ...
    slcan_tx_info_to_bus_g.head = (slcan_tx_info_to_bus_g.head + 1u) % SLCAN_TX_FIFO_NUM;
#else
    slcan_byte_packets.CAN_TxHeader.ExtId = can_id;
    slcan_byte_packets.CAN_TxHeader.DLC = can_dlc;
    slcan_byte_packets.CAN_TxHeader.RTR = CAN_RTR_DATA;
    slcan_byte_packets.CAN_TxHeader.IDE = CAN_ID_EXT;
    osMessageQueuePut( can_info_to_pc_qHandle, &slcan_byte_packets, 0u, 0u );
#endif
    return HAL_OK;
}
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
    ...
    uint8_t can_data[8];
    slcan_str_info_packets_t info_to_pc;
    ...
    {
        info_to_pc.slcan_str[0] = 't';
        sprintf((char *)&info_to_pc.slcan_str[1], "%03X", (uint16_t)CAN_RxHeader.StdId);
        sprintf((char *)&info_to_pc.slcan_str[4], "%d", (uint8_t)dlc);
        data_of_start = 5u;
    }
    else
    {
        info_to_pc.slcan_str[0] = 'T';
        sprintf((char *)&info_to_pc.slcan_str[1], "%08X", (uint16_t)CAN_RxHeader.ExtId);
        sprintf((char *)&info_to_pc.slcan_str[9], "%d", (uint8_t)dlc);
        data_of_start = 10u;
    ...
    {
        sprintf((char *)&info_to_pc.slcan_str[data_of_start + i*2], "%02X", can_data[i]);
    }
    info_len = dlc * 2 + data_of_start;
    info_to_pc.slcan_str[ info_len ] = '\r';
    info_len++;
#ifndef USE_FREERTOS
    uint8_t head;
...
#else
    info_to_pc.str_len = info_len;
    osMessageQueuePut( slcan_info_to_pc_qHandle, &info_to_pc, 0u, 0u );
#endif
}
