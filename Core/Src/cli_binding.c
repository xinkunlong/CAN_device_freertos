...
void cli_print_stack_info(EmbeddedCli *cli, char *args, void *context)
{
	(void)cli;
	(void)args;
	(void)context;
	extern osThreadId_t pc_slcanHandle;
	extern osThreadId_t can_info_to_pcHandle;
	extern osThreadId_t Task_CliHandle;
	extern osThreadId_t can_info_to_busHandle;
	cli_printf("pc_slcanHandle:%d" ,osThreadGetStackSpace( pc_slcanHandle ));
	cli_printf("can_info_to_pcHandle:%d" ,osThreadGetStackSpace( can_info_to_pcHandle ));
	cli_printf("Task_CliHandle:%d",osThreadGetStackSpace( Task_CliHandle ));
	cli_printf("can_info_to_busHandle:%d" ,osThreadGetStackSpace( can_info_to_busHandle ));

}


void cli_print_task_info(EmbeddedCli *cli, char *args, void *context)
{
	(void)cli;
	(void)args;
	(void)context;
	...
	extern osThreadId_t Task_CliHandle;
	extern osThreadId_t can_info_to_busHandle;
	cli_printf("osThreadInactive: 0, osThreadReady: 1, osThreadRunning: 2, osThreadBlocked: 3,osThreadTerminated: 4");
	cli_printf("pc_slcanHandle:%d" ,osThreadGetState( pc_slcanHandle ));
	cli_printf("can_info_to_pcHandle:%d" ,osThreadGetState( can_info_to_pcHandle ));
	cli_printf("Task_CliHandle:%d",osThreadGetState( Task_CliHandle ));
	cli_printf("can_info_to_busHandle:%d" ,osThreadGetState( can_info_to_busHandle ));
...
void initCliBinding() {
    ...
    CliCommandBinding stackinfo_binding = {
            .name = "print-stack-info",
            .help = "print stack info",
            .tokenizeArgs = true,
            .context = NULL,
            .binding = cli_print_stack_info
    };
    CliCommandBinding taskinfo_binding = {
            .name = "print-task-info",
            .help = "print task info",
            .tokenizeArgs = true,
            .context = NULL,
            .binding = cli_print_task_info
    };
    EmbeddedCli *cli = getCliPointer();
    embeddedCliAddBinding(cli, clear_binding);
    embeddedCliAddBinding(cli, baudrate_binding);
    embeddedCliAddBinding(cli, stackinfo_binding);
    embeddedCliAddBinding(cli, taskinfo_binding);
}
